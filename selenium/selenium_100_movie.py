...
def scrape_page(url, condition, locator):
    ...
    try:
        ...
        logging.error('error occurred while scraping %s', url, exe_info=True)
...
import json
RESYLTS_DIR = 'selenium-100movies-results'
exists(RESYLTS_DIR) or makedirs(RESYLTS_DIR)

def save_data(data):
    name = data.get('name')
    data_path = f'{RESYLTS_DIR}/{name}.json'
